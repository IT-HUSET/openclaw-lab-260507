...
usage() {
  cat >&2 <<'EOF'
Usage:
  ./scripts/native-multi-gateway/status.sh <profile>

Example:
  ./scripts/native-multi-gateway/status.sh 1
  ./scripts/native-multi-gateway/status.sh p01
EOF
  exit 1
}
...
openclaw --profile "$PROFILE" gateway status --deep
openclaw --profile "$PROFILE" gateway probe
